...
const openMessage = (type: MessageType, config: MessageConfig) => {
  ...
  const existingIndex = messageList.value.findIndex(item => item.id === nowId);
  if (existingIndex!==-1) {
    Object.assign(messageList.value[existingIndex], { ...config,type, id: nowId, resetFlag: !messageList.value[existingIndex].resetFlag });
  } else {
...
const messageMethod = messageType.reduce((acc, type) => {
  acc[type] = (config: MessageConfig) => openMessage(type, config);
  return acc;
}, {} as Record<MessageType, (config: MessageConfig) => {
  close:()=>void;
}>)
